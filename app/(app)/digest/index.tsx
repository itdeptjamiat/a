      ...
      <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <KidsCategoryCard
                ...
                icon={item.icon}
                colors={item.colors}
                onPress={() => console.log('Category tapped:', item.title)}
              />
            )}
        ...
        <View style={styles.section}>
          ...
          <FlatList
            horizontal
            showsHorizontalScrollIndicator={false}
            data={digests.slice(0, 5)} // Show first 5 featured
            keyExtractor={(item) => String(item.mid)}
            renderItem={({ item }) => (
              <DigestCard digest={item} onPress={handleDigestPress} />
            )}
        ...
        <View style={styles.section}>
          ...
          <FlatList
            horizontal
            showsHorizontalScrollIndicator={false}
            data={digests.slice(5, 10)} // Show next 5 popular
            keyExtractor={(item) => String(item.mid)}
            renderItem={({ item }) => (
              <DigestCard digest={item} onPress={handleDigestPress} />
            )}
        ...
        <View style={styles.section}>
          ...
          <FlatList
            horizontal
            showsHorizontalScrollIndicator={false}
            data={digests.slice(10, 15)} // Show next 5 new releases
            keyExtractor={(item) => String(item.mid)}
            renderItem={({ item }) => (
              <DigestCard digest={item} onPress={handleDigestPress} />
            )}
